...
class TokenManager:
    ...
    def _get_token_sync(self, org_id: str) -> Optional[Dict[str, Any]]:
        ...
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.execute(
                "SELECT access_token, instance_url, expiry FROM tokens WHERE org_id = ?", (org_id,)
            )
            row = cursor.fetchone()
    ...
    def _save_token_sync(
        self, org_id: str, encrypted_token: str, instance_url: str, expiry: datetime
    ):
...
class SalesforceConnector(BaseCRMConnector):
    ...
    async def _query_rest(self, soql: str) -> pd.DataFrame:
        ...
        records = []
        next_url = (
            f"{self.config.instance_url}/services/data/{self.config.api_version}/query?q={soql}"
        )
